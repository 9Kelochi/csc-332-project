...
def is_duplicate_id(table, id_column, value):
    """
    Checks if a given ID value already exists in the specified table and column.

    Args:
        table (str): Table name (e.g., "invitations").
        id_column (str): Column name to check (e.g., "invite_id").
        value (str): The ID value to check for duplication.

    Returns:
        bool: True if duplicate exists, False otherwise.
    """
    conn = sqlite3.connect("token_terminator.db")  # or "users.db" if still using that
    cursor = conn.cursor()
    query = f"SELECT 1 FROM {table} WHERE {id_column} = ? LIMIT 1"
     ...
     if "corrected_text" in st.session_state:
         ...
         if username is not None:
             ...
             if st.button("Save File") and File_name:
                 ...
                 while is_duplicate_id("files", "file_id", save_id):
                  save_id = generate_random_id()
                 cursor.execute("SELECT ID FROM users WHERE username = ?", (username,))
...
def sent_to_collab(invite_id):
    ...
    if row:
        owner_id, collaborator_id, file_id = row  # fixed line
     ...
     if st.button("Send Invitation"):
         ...
         if file_result:
             ...
             if invited_user_exists:
                 ...
                 while is_duplicate_id("invitations", "invite_id", invite_id):
                     invite_id = generate_random_id()
                 now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def register():
     ...
     if st.button("Register"):
         if same_username(username):
             ...
             while is_duplicate_id("users", "ID", register_id):
              register_id = generate_random_id()
             try:
                 conn = sqlite3.connect("token_terminator.db")
                 cursor = conn.cursor()
